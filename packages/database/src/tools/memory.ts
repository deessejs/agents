...
import { z } from "zod";
import { searchMemories, writeMemory, updateMemory, forgetMemory, readCoreMemory } from "../queries.js";
...
function resolvePath(path: string): { tier: string; metadata: Record<string, unknown> } {
  ...
  if (dateMatch) {
    return { tier: "archival", metadata: { date: dateMatch[1] } };
  }
  ...
  async execute(input) {
    ...
    switch (command) {
      case "view": {
        ...
        const { tier: resolvedTier } = resolvePath(path);
        const results = await readCoreMemory();
        return results;
      }
